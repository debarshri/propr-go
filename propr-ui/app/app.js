...
appProp.config(['$locationProvider', '$routeProvider',
   ...
   .when('/delete-property', {
        template: '<div ng-controller="DeletePropertyController" class="form-group">'+
                                        '<form novalidate><legend>App Name</legend>'+
                                        '<input type="text" ng-model="property.property" class="form-control" placeholder="property name" />'+
                                        '<br /><input type="button" ng-click="delete(property)" value="Delete Property" class="btn btn-default" />'+
                                        '</form><pre ng-show="result.property != null"  ng-model="result">{{result.property}}</pre></div></div>' })
...
appProp.controller('ExampleController', function ExampleController($scope, $http) {
    ...
    function successCallback(response){
        if(response.data === ""){
            $scope.result.property = "No properties"
        } else {
...
appProp.controller('DeletePropertyController', function CreatePropertyController($scope, $http) {
    ...
    $scope.delete = function(property) {
        $http.post('/delete/'+property.property).then(successCallback, errorCallback);
    };
